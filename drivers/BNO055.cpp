...
ssize_t BNO055::init() {
	...
	ThisThread::sleep_for(19ms);  // switch to config mode takes 19ms according to datasheet
	...
	ThisThread::sleep_for(7ms);  // switch to operating mode takes 7ms according to datasheet
...
int BNO055::write_config(char* buf) {
	...
	ThisThread::sleep_for(1ms); // dunno if we actually need this...
